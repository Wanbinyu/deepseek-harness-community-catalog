...
| [dsh-plugin-git-inspect](https://github.com/Wanbinyu/dsh-plugin-git-inspect) | Read-only `git_status`, `git_diff`, and `git_log` tools. | Harness `0.1.0-rc.x`; Node.js `>=22.19.0` | `dsh plugin --profile web add github:Wanbinyu/dsh-plugin-git-inspect` |

## Companion Tools

| Project | Type | What it provides | Use |
| --- | --- | --- | --- |
| [dsh-launcher](https://github.com/Wanbinyu/dsh-launcher) | Windows CLI tool | Short `dsh` and `deepseek` commands for starting the Harness Web profile and opening the browser. | Clone the repository and run `install.ps1`. |
...
The machine-readable catalog is [`plugins.json`](plugins.json). For projects classified as bundles, the minimum checks are:
